...
public final class GameConfig {
    ...
    public static final float PLAYER_SPEED = 190f;   // px/s
    public static final float ENEMY_SPEED = 130f;    // px/s
    public static final float BULLET_SPEED = 420f;   // px/s

    public static final float PLAYER_FIRE_COOLDOWN = 0.16f;
    public static final float ENEMY_FIRE_COOLDOWN = 0.9f;

    // IMPORTANT: 60 Hz fixed sim to keep it fast/playable
    public static final float FIXED_DT = 1f / 60f;

    // IMPORTANT: only clamp huge stalls (alt-tab etc.), not normal slow frames
    public static final float MAX_FRAME_DT = 0.25f;

    // Avoid spiral of death: max sim steps per rendered frame
